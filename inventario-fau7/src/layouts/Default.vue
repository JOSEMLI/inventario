<template>
  <div>
    <v-app-bar color="primary" class="flex-grow-0 "  dark>
      ...
      <v-toolbar-title> Inventarios Fau </v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn icon>
        <v-icon>mdi-logout</v-icon>
      </v-btn>
    </v-app-bar>
    <v-navigation-drawer  v-model="drawer" class="fondo">
        <v-list-item>
            <v-list-item-title>
                <v-avatar>
            ...
            <v-list-item-sutitle>Usuario</v-list-item-sutitle>
        </v-list-item>
        <v-divider></v-divider>
        <v-list dense nav>
            <v-list-item v-for="(item, index) in items" :key="index" @click="navigate(item.value)" color="blue-lighten-5" variant="text" rounded="shaped" >
...
export default {
  ...
  data: () => ({
    drawer: true,
    items: [
    { title: "Home", icon: "mdi-home", value: '' },
    { title: "Nivel", icon: "mdi-escalator-up", value: 'about' },
    { title: "Nivel", icon: "mdi-escalator-up", value: 'nivel' },
    { title: "Mesas", icon: "mdi-table-furniture", value: 'mesas' },
    { title: "Sillas", icon: "mdi-table-chair", value: 'sillas' },
    { title: "Configurar", icon: "mdi-cog", value: 'configuracion' },
    ]
  }),
  methods: {
    navigate(value) {
      // Realiza la navegación basada en el valor
